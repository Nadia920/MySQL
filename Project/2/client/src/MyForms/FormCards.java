...
public class FormCards extends javax.swing.JFrame {
    //CardsTableInf modelTableCards = new CardsTableInf();
    private TableModel cards;
    ...
    public FormCards() {
       /* try {
         ...
         try {
            //setCardsModel ();
            setCardsModel("SELECT * FROM cards");
        } catch (IOException ex) {
            //Logger.getLogger(MainAdminFrame.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public FormCards(String number_card, String validity, String CVV, String card_balance, String status_card  ){}
    SocketStream socketstream = new SocketStream();

    public void setCardsModel (String executeStr) throws IOException{
        ArrayList<Cards> cardsInf = new ArrayList<>();
        ...
        for(int i = 0; i < count; i++){
            ...
            String status_card = socketstream.getString();
            /*FormCards card = new FormCards(number_card, payment_information, amount, status);
            cards.add(card);*/
             cardsInf.add(new Cards(number_card, validity, CVV, card_balance, status_card));
        }
	//payment = new PaymentsTableInf(payments);
        cards = new CardsTableInf(cardsInf);
    }
    ...
    private void initComponents() {
        ...
        Cards.setModel(cards);
        jScrollPane3.setViewportView(Cards);
    ...
    private void ButtonPieChartMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ButtonPieChartMouseClicked
        ...
        for (int i = 0; i < count; i++){
            String name = (String)Cards.getValueAt(i, 0);
            float dd = (float)Cards.getValueAt(i, 2);
    ...
    private void ButtonCreateReportMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ButtonCreateReportMouseClicked
        ...
        try {
            ...
            for (int i = 0; i < count; i++){
                String name = (String)Cards.getValueAt(i, 0);
                float d1 = (float)Cards.getValueAt(i, 1);
                float d2 = (float)Cards.getValueAt(i, 2);
                float d3 = (float)Cards.getValueAt(i, 3);
                float d4 = (float)Cards.getValueAt(i, 4);
            ...
            Logger.getLogger(FormCards.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void ButtonDetailsMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ButtonDetailsMouseClicked
        if(Cards.getSelectedRowCount() == 0){
            JOptionPane.showMessageDialog(null, "Необходимо выбрать запись!",
                "Ошибка!",JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        String name = (String)Cards.getValueAt(Cards.getSelectedRow(), 0);
...
///////////////////////////сортировка///////////////////////////
    ...
    private javax.swing.JTable Cards;
